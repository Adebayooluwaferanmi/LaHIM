...
ENV_FILE="${LAHIM_DOCKER_ENV_FILE:-${ROOT_DIR}/.env.docker}"
...
if [ "${1:-}" = "--help" ]; then
  cat <<EOF
Usage: ./scripts/docker-up-native.sh [service...]

Starts the boot-first LaHIM core stack from a Linux-native copy of the repository.
Defaults to: postgres redis couchdb lahim-server staff-frontend

Supported environment variables:
  LAHIM_NATIVE_DIR         Target native workspace (default: /tmp/lahim-native-\$USER)
  LAHIM_DOCKER_ENV_FILE    Docker env file (default: ./.env.docker)
...
if [ "$#" -eq 0 ]; then
  SERVICES=("postgres" "redis" "couchdb" "lahim-server" "staff-frontend")
else
  SERVICES=("$@")
fi
...
if [ ! -f "${ENV_FILE}" ]; then
  ...
  echo "Create it with: cp ${ROOT_DIR}/.env.docker.example ${ROOT_DIR}/.env.docker" >&2
  exit 1
fi
...
echo "Starting services from ${NATIVE_DIR}: ${SERVICES[*]}"
DOCKER_BUILDKIT="${DOCKER_BUILDKIT_VALUE}" \
  "${DOCKER_BIN}" compose \
  -f "${NATIVE_DIR}/docker-compose.yml" \
  --env-file "${ENV_FILE}" \
  up -d "${SERVICES[@]}"
